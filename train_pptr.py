...
# from datasets.AS_base_baseline import SegDataset
from datasets.AS_base_segment_no2d import SegDataset
...
def weight_count(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq):
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq,weight):
    model.train()
    ...
    for clip, target,image_2d,segment_feature in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # output = model(clip)
        output = model(clip,segment_feature)
        ...
        # loss = Wce(output.permute(0,2,1), target,weight)
###### v2
        # if epoch < 40:
        #     # print('epoch',epoch)
        #     loss = loss2
...
#####
    ...
    # torch.save(state, '/mnt/petrelfs/jinglinglin/4D_HOI/Action_seg/submit_npy/s5.pth')

def evaluate(model, criterion, data_loader, device, len_test,pre,eval_max):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    acc_list = []
    ...
    with torch.no_grad():
        ...
        for clip, target, image_2d , segment_feature in metric_logger.log_every(data_loader, 20, header):
            ...
            output_new2 = output_new
            output_new3 = output_new2
    ...
    np.save('/mnt/petrelfs/jinglinglin/4D_HOI/Action_seg/submit_npy/s5.npy', pre_out)
    return total_acc
...
#######
    # count_loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=args.workers, pin_memory=True)
    # co = []
    # a =  weight_count(model, criterion, optimizer, lr_scheduler, count_loader, device, 1, args.print_freq)
    # for k ,v in a.items():
    #     co.append(v)
...
########
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, args.print_freq, weight)
        ...
        if epoch > 0:
            pre = []
            acc = max(acc, evaluate(model, criterion, data_loader_test, device, len(dataset_test),pre,eval_max))
            eval_max = acc
