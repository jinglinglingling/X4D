...
def train_one_epoch(model,CLIP,temporal_tf1,temporal_tf2, resnet18,resnet18_2, mix_model,mlp_m,mlp_m_f,mlp_m_l, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq,weight):
    ...
    for clip, target,image_2d, tempgrad, segment_feature in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # image_slide_feature = torch.zeros(img_fea.shape)
        # tem_slide_feature = torch.zeros(tem_fea.shape)

        # for i in range(0,150):
        #     if i == 0:
        #         image_slide_feature[:,i,:] = img_fea[:,i,:] + img_fea[:,i,:] + img_fea[:,i+1,:]
        #         tem_slide_feature[:,i,:] = tem_fea[:,i,:] + tem_fea[:,i,:] + tem_fea[:,i+1,:]
        #     elif i == 149:
        #         image_slide_feature[:,i,:] = img_fea[:,i-1,:] + img_fea[:,i,:] + img_fea[:,i,:]
        #         tem_slide_feature[:,i,:] = tem_fea[:,i-1,:] + tem_fea[:,i,:] + tem_fea[:,i,:]
        #     else:
        #         image_slide_feature[:,i,:] = img_fea[:,i-1,:] + img_fea[:,i,:] + img_fea[:,i+1,:]
        #         tem_slide_feature[:,i,:] = tem_fea[:,i-1,:] + tem_fea[:,i,:] + tem_fea[:,i+1,:]

        # # mix_feature = torch.cat([image_slide_feature,tem_slide_feature],3)
        # mix_feature = image_slide_feature + tem_slide_feature
        # print('mix',mix_feature.shape)
        mix_feature = img_fea + tem_fea
        mix_feature = mix_feature.to(device)

        C = mix_feature.shape[3]
        # print(C)
        # print('11111',mix_feature.shape)
...
###########################
        ...
        loss_predict_for_cross = loss_predict_for / B
        ...
        loss_predict_back_cross = loss_predict_back / B
...
def main(args):
    ...
    if torch.cuda.device_count() > 1:
        ...
        # CLIPModel = nn.DataParallel(CLIPModel)
...
def parse_args():
    ...
    parser.add_argument('--epochs', default=65, type=int, metavar='N', help='number of total epochs to run')
