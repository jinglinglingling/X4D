...
class P4Transformer(nn.Module):
    def __init__(self, radius, nsamples, spatial_stride,                                # P4DConv: spatial
        ...
        self.tube_embedding = P4DConv(in_planes=0, mlp_planes=[dim], mlp_batch_norm=[False], mlp_activation=[False],
                                  ...
                                  temporal_kernel_size=temporal_kernel_size, temporal_stride=temporal_stride, temporal_padding=[2, 2],
    ...
    def forward(self, input):
        ...
        # 4d BACKBONE
        # [B, L, N, 3]     b * 150 * 2048 * 3
        xyzs, features = self.tube_embedding(input)  # [B, L, n, 3], [B, L, C, n] 

        features = features.transpose(2, 3)  # B ,L , n, C    b * 150 * 64 * 2048
        ...
        for t, xyz in enumerate(xyzs):
            ...
            xyzts.append(xyzt)
        xyzts = torch.stack(tensors=xyzts, dim=1)                 # b * 150 * 64 * 4
        ...
        xyzts = self.pos_embedding(xyzts.permute(0, 2, 1)).permute(0, 2, 1)                     # b * l*n * 2048  position embedding
