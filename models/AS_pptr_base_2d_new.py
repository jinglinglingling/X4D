...
class fusion_net(nn.Module):
    def __init__(self ):                                            
        ...
        self.pos_embedding = nn.Conv1d(in_channels=1, out_channels=2048, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.learning_position = LearnedPositionEncoding(d_model=2048,max_len=150)
    ...
    def forward(self, input_3d, input_2d,label,xyzt,mode):
        ...
        # ### SAT pe
        # t = list(range(0,150)) 
        # t_emb = torch.zeros((input_3d.shape[0], input_3d.shape[1],1))
        # for i in t:
        #     t_emb[:,i,:] = t[i]
            
        # # print(t_emb)

        # t_emb = t_emb.to(input_3d.device)
        # pe_2d = self.pos_embedding(t_emb.permute(0, 2, 1)).permute(0, 2, 1)
...
class PrimitiveTransformer(nn.Module):
    ...
    def forward(self, input): 
        ...
        for t, xyz in enumerate(xyzs):
            ...
            # xyz_0 = torch.zeros((xyz.shape)).to(device)
            # xyzt = torch.cat(tensors=(xyz_0, t), dim=2)
        ...
        B = xyzts.shape[0]
        xyz_test = torch.reshape(input=xyzts, shape=(B*150, 64, 2048))
        xyz_test = xyz_test.permute(0, 2, 1)
        xyz_test = F.adaptive_max_pool1d(xyz_test, (1))
        xyz_test = torch.reshape(input=xyz_test, shape=(B, 150, 2048))

        ######
        xyzts_before = xyz_test.clone()
