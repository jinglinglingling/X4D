...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        # print('temporal_size',self.temporal_kernel_size % 2 )
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            anchor_idx = pointnet2_utils.furthest_point_sample(xyzs[t], npoints//self.spatial_stride)                               # (B, N//self.spatial_stride)  #FPS采样
            # print('anchor_idx',anchor_idx.shape)
            anchor_xyz_flipped = pointnet2_utils.gather_operation(xyzs[t].transpose(1, 2).contiguous(), anchor_idx)                 # (B, 3, N//self.spatial_stride)
            # print('anchor_xyz_flipped',anchor_xyz_flipped.shape)
            anchor_xyz_expanded = torch.unsqueeze(anchor_xyz_flipped, 3)                                                            # (B, 3, N//spatial_stride, 1)
            anchor_xyz = anchor_xyz_flipped.transpose(1, 2).contiguous()                                                            # (B, N//spatial_stride, 3) 32 * 64 * 3

            new_feature = []
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                neighbor_xyz = xyzs[i]     
                # print('neighbor_xyz',neighbor_xyz.shape)
                idx = pointnet2_utils.ball_query(self.r, self.k, neighbor_xyz, anchor_xyz)                                          # 32*64*32  neighbor number

                neighbor_xyz_flipped = neighbor_xyz.transpose(1, 2).contiguous()                                                    # (B, 3, N)
                neighbor_xyz_grouped = pointnet2_utils.grouping_operation(neighbor_xyz_flipped, idx)                                # (B, 3, N//spatial_stride, k)   32*3*64*32
                # print('neighbor_xyz_grouped',neighbor_xyz_grouped.shape)
                xyz_displacement = neighbor_xyz_grouped - anchor_xyz_expanded                                                       # (B, 3, N//spatial_stride, k)  32*3*64*32
                t_displacement = torch.ones((xyz_displacement.size()[0], 1, xyz_displacement.size()[2], xyz_displacement.size()[3]), dtype=torch.float32, device=device) * (i-t) # 32*1*64*32
                displacement = torch.cat(tensors=(xyz_displacement, t_displacement), dim=1, out=None)                               # (B, 4, N//spatial_stride, k) .  32*4*64*32
                displacement = self.conv_d(displacement)                                                                            # 2*2048*64*32                      
                ...
                if self.in_planes != 0:
                    ...
                    feature = displacement                                                                                          #(2,2048,64,32)

                feature = self.mlp(feature)
                # print('feature',feature.shape)
                if self.spatial_pooling == 'max':
                    feature = torch.max(input=feature, dim=-1, keepdim=False)[0]                                                        # (B, out_planes, n) 32*2048*64
            ...
            new_feature = torch.stack(tensors=new_feature, dim=1)                                  # 32 * 3 * 2048 * 64
            if self.temporal_pooling == 'max':
                new_feature = torch.max(input=new_feature, dim=1, keepdim=False)[0]                 # 32 * 2048 * 64
        ...
        new_xyzs = torch.stack(tensors=new_xyzs, dim=1)                                              # 32 * 150 * 64 * 3                   
        new_features = torch.stack(tensors=new_features, dim=1)                                       # 32 * 150 * 2048 * 64           

        return new_xyzs, new_features , anchor_idx
