...
def plott(points):
    ...
    ax.axis()
    plt.show()
    # plt.savefig('/nvme/jinglinglin/Linglin_x/Action_seg/HOI4D_ActionSeg-main/vis.png')
    # return frame1
...
def viz_matplot(points):
    ...
    ax.axis()
    # plt.show()
    # plt.savefig('/nvme/jinglinglin/Linglin_x/Action_seg/HOI4D_ActionSeg-main/vis1.png')
    return frame1
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq):
    ...
    for clip, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # gif.save(frames,'/nvme/jinglinglin/Linglin_x/Action_seg/HOI4D_ActionSeg-main/tu_100.gif',duration=10)
...
def main(args):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
...
def parse_args():
    ...
    parser.add_argument('--lr-milestones', nargs='+', default=[20, 35, 50,70], type=int, help='decrease lr on milestones')
...
if __name__ == "__main__":
    ...
    # bbbb = np.load('/nvme/jinglinglin/Linglin_x/Action_seg/HOI4D_ActionSeg-main/output.npy',allow_pickle=True)
    # ccc = np.max(bbbb)
    # exit()
