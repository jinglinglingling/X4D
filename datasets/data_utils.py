...
class PointcloudScale(object):
    ...
    def __call__(self, pc):
        ...
        points *= scaler
        points = points + center
        # pc[0] = torch.from_numpy(points).float()
        pc[0] = points.to(torch.float32)
        return pc
...
class PointcloudRandomInputDropout(object):
    def __init__(self, max_dropout_ratio=0.5, p=1):
    ...
    def __call__(self, pc):
        ...
        for i in range (pc.shape[0]):
            ...
            xx = np.random.random((points.shape[0]))
            # drop_idx = np.where(np.random.random((pc.shape[0],pc.shape[1])) <= dropout_ratio)[0]
            drop_idx = (np.where( xx < dropout_ratio))

            # drop_idx1 = xx(np.where( xx[:] < dropout_ratio))
            # drop_idx = 
            if len(drop_idx) > 0:
                pcpc = points.copy()
                # pc[:,drop_idx,:] = pc[:,0,:]  # set to the first point
                # pcpc[:, drop_idx,:] = points[:,0,:]
                pcpc[drop_idx,:] = points[0,:]
...
class PointcloudNormalize(object):
    ...
    def pc_normalize(self, point):
        ...
        m = np.max(np.sqrt(np.sum(pc ** 2, axis=2))) # 求取长轴的的长度
        pc = pc / m  # 依据长轴将点云归一化到 (-1, 1)
