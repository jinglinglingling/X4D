...
import models.AS_pptr_base_2d_new_segment as Models
...
def train_one_epoch(model,resnet18,mix_model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq,weight):
    model.train()
    resnet18.train()
    mix_model.train()
    ...
    # img_2d_list = []
    for clip, target,image_2d,segment_feature in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # output_img = output2.reshape(B,L,2048) 

        segment_feature = segment_feature.to(device)
        clip, target = clip.to(device), target.to(device)
        output_3d , xyzt = model(clip,segment_feature)

        output_fusion , fusion_feature= mix_model(output_3d,output_img,target,xyzt,'train')



        output = output_fusion[:,:150,:] 
        pre_2d = output_fusion[:,150:,:] 
        

        Feature_3d = fusion_feature[:,:150,:] 
        Featur_2d = fusion_feature[:,150:,:] 
        ...
        # bat = bat.to(device)
        # sim2d3d_loss = contrastive_loss(Featur_2d, Feature_3d, bat)

        # contras_loss =  F.mse_loss(Feature_3d ,Featur_2d)
...
#     torch.save(state, '/mnt/petrelfs/jinglinglin/4D_HOI/Experiment/Sbatch_test/output/Last_day/'+ version)



def evaluate(model,resnet18,mix_model, criterion, data_loader, device, len_test,pre,eval_max,epoch):
    model.eval()
    resnet18.eval()
    mix_model.eval()
    ...
    with torch.no_grad():
        ...
        for clip, target, image_2d , segment_feature in metric_logger.log_every(data_loader, 20, header):
            ...
            output_fusion , fusion_feature = mix_model(output_3d,output_img,target,xyzt,'test')
            ...
            loss = loss_3d + loss3_2d  #+ 0.5 * loss1 + 0.5 * loss1_1 #+ sim2d3d_loss
    ...
    # np.save('/mnt/petrelfs/jinglinglin/4D_HOI/Experiment/Sbatch_test/output/Last_day/S11_nopost/%s.npy'%(epoch+20), pre_out)
    return total_acc
...
########
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_one_epoch(model,resnet18,mix_model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, args.print_freq, weight)
        # lr_scheduler.step()
        # if epoch > 55:
        pre = []
        acc = max(acc, evaluate(model,resnet18,mix_model, criterion, data_loader_test, device, len(dataset_test),pre,eval_max,epoch))
        eval_max = acc
